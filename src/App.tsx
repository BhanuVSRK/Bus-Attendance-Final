...
const Card = ({ children, className, title, ...props }: any) => (
  <div 
    {...props}
    className={cn("w-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden", className)}
  >
    {title && (
...
function StudentManagement() {
  ...
  useEffect(() => {
    fetchStudents();
    fetch('/api/buses').then(res => res.json()).then(data => setBuses(data.sort((a: any, b: any) => a.bus_number.localeCompare(b.bus_number))));
  }, []);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <select required value={formData.bus_number} onChange={e => setFormData({...formData, bus_number: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-slate-200">
                    ...
                    {[...buses].sort((a, b) => a.bus_number.localeCompare(b.bus_number)).map(b => <option key={b.bus_number} value={b.bus_number}>{b.bus_number} - {b.route}</option>)}
                  </select>
                </div>
                <div>
      ...
      <Card className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {[...buses].sort((a, b) => a.bus_number.localeCompare(b.bus_number)).map((b) => (
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...buses].sort((a, b) => a.bus_number.localeCompare(b.bus_number)).map((bus) => (
...
function DriverScanner({ user, setView }: { user: User, setView: (view: string) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full space-y-6 text-white">
      ...
      <Card title="Attendance Scanner" className="w-full bg-slate-900 border-slate-800 text-white">
            ...
            {scanning === 'camera' ? (
              <div className="relative w-full h-full">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-64 h-64 border-4 border-indigo-500 rounded-2xl shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]"></div>
                </div>
              </div>
            ) : (
              <button 
                onClick={() => setScanning('camera')}
                className="flex flex-col items-center gap-3 text-white bg-indigo-600 px-12 py-8 rounded-xl"
              >
                <Camera size={64} />
                <span className="font-bold text-xl">Start Scanning</span>
              </button>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            >
              ...
              {[...buses].sort((a, b) => a.bus_number.localeCompare(b.bus_number)).map(b => <option key={b.bus_number} value={b.bus_number}>{b.bus_number}</option>)}
            </select>
          </div>
          <button 
